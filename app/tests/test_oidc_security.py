...
def _settings() -> Settings:
    return Settings(
        ...
        casdoor_client_id="info-admin-client",
        casdoor_client_secret="test-only-secret",
        casdoor_redirect_uri="https://info.example.test/api/auth/callback",
        casdoor_application="sunmoonai-info-admin",
        frontend_base_url="https://info.example.test",
...
def _token(key: RSAKey, **overrides) -> str:
    now = int(time.time())
    claims = {
        "iss": "https://identity.example.test/.well-known/sunmoonai-info-admin",
        "sub": "user-123",
        "aud": "info-admin-client",
        "iat": now,
        "exp": now + 300,
        "nonce": "nonce-123",
        "name": "Test User",
        ...
        if request.url.path.endswith("/openid-configuration"):
            return httpx.Response(
                ...
                json={
                    ...
                    "jwks_uri": "https://identity.example.test/.well-known/sunmoonai-info-admin/jwks",
                },
            )
...
async def test_backchannel_transport_preserves_public_issuer_and_host() -> None:
    ...
    assert claims["iss"] == "https://identity.example.test/.well-known/sunmoonai-info-admin"
    assert paths == [
        "/.well-known/sunmoonai-info-admin/openid-configuration",
        "/api/login/oauth/access_token",
        "/.well-known/sunmoonai-info-admin/jwks",
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"aud": ["info-admin-client", "research-admin-client"]},
        {"nonce": "wrong-nonce"},
