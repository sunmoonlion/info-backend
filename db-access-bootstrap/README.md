...
用于架构 v2 的统一 `info-backend` 数据库接入脚手架。Admin 与 Web
...
- **`DBCTL_BIN`**：未设置时解析为 **`info-backend/db-provisioner/bin/dbctl`**（相对 `db-access-bootstrap` 的上一级）；使用仓库外二进制时用环境变量覆盖（Windows 可用 `/c/...`）。
...
```bash
cd info-backend/db-access-bootstrap
